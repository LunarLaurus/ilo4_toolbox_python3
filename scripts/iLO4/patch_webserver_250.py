...
PATCH1 = {"offset": 0x1410, "size": len("/backd00r.htm"), "prev_data": b"/html/IRC.exe", "patch": b"/backd00r.htm", "decode": None}
PATCHES.append(PATCH1)

# Patch GET handler address: 0x29278 -> 0x193CD4
PATCH2 = {"offset": 0x188B18, "size": 4, "prev_data": b"78920200", "patch": b"D43C1A00", "decode": "hex"}
...
PATCH4 = {
    "offset": 0xFBFE,
    "size": len("Server/1.30"),
    "prev_data": b"Server/1.30",
    "patch": ("SSTIC/%02d.%02d" % (now.hour, now.minute)).encode(),
    "decode": None
}
PATCHES.append(PATCH4)

# Patch query string decoding bug
PATCH5 = {"offset": 0x5D534, "size": 4, "prev_data": b"25640000", "patch": "A8CE0400", "decode": "hex"}
PATCHES.append(PATCH5)
PATCH6 = {"offset": 0x5D1A4, "size": 4, "prev_data": b"E21F8FE2", "patch": "88139FE5", "decode": "hex"}
PATCHES.append(PATCH6)

# Patch verbosity
PATCH7 = {"offset": 0x3A6D4, "size": 1, "prev_data": b"00", "patch": "09", "decode": "hex"}
...
data = bytearray(elf_data[webserver_offset:webserver_offset+webserver_size])
...
for patch in PATCHES:
    ...
    if "prev_data" in patch:
        if patch["decode"] is None:
            ...
            prev_data = bytes.fromhex(patch["prev_data"])
        if check_data != prev_data:
            print("[-] Error, bad file content at offset 0x%x" % patch["offset"])
            sys.exit(1)

    if patch["decode"] is None:
        patch_data = patch["patch"]
    else:
        patch_data = bytes.fromhex(patch["patch"])

    data[patch["offset"]:patch["offset"]+patch["size"]] = patch_data
